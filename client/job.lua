    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                ...
                TriggerServerEvent('fbi:server:TakeOutImpound',vehicle.plate)
                closeMenuFull()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'fbi' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "fbi:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    local impoundMenu = {
        {
            header = "Vehicles confiscated",
            isMenuHeader = true
        }
    }
    QBCore.Functions.TriggerCallback("fbi:GetImpoundedVehicles", function(result)
        local shouldContinue = false
        if result == nil then
            QBCore.Functions.Notify("No vehicles seized", "error", 5000)
        else
            shouldContinue = true
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "fbi:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
RegisterNetEvent('fbi:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('fbi:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(data)
    TriggerServerEvent('fbi:server:showFingerprintId', FingerPrintSessionId)
end)

RegisterNetEvent('fbi:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("fbi:server:SendEmergencyMessage", coords, message)
    TriggerEvent("fbi:client:CallAnim")
end)

RegisterNetEvent('fbi:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('fbi:client:CallAnim', function()
...
RegisterNetEvent('fbi:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("fbi:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('fbi:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "fbi" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('fbi:GetPlayerStatus', function(result)
                    if result then
...
RegisterNetEvent("fbi:client:VehicleMenuHeader", function (data)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent("fbi:client:ImpoundMenuHeader", function (data)
...
RegisterNetEvent('fbi:client:TakeOutImpound', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['impound'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('fbi:client:TakeOutVehicle', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['vehicle'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('fbi:client:EvidenceStashDrawer', function(data)
...
CreateThread(function()
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "fbi" then
            local pos = GetEntityCoords(PlayerPedId())
                ...
                if #(pos - v) < 5 then
                    ...
                    if #(pos - v) < 1.5 then
                        if not onDuty then
                            ...
                            DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Exit Service")
                        end
                        if IsControlJustReleased(0, 38) then
                            onDuty = not onDuty
                            TriggerServerEvent("fbi:server:UpdateCurrentCops")
                            TriggerServerEvent("QBCore:ToggleDuty")
                            TriggerServerEvent("fbi:server:UpdateBlips")
                        end
                            ...
                            exports['qb-menu']:showHeader({
                                {
                                    ...
                                    params = {
                                        event = 'fbi:client:EvidenceStashDrawer',
                                        args = {
                                            currentEvidence = k
                                        }
                                    }
            ...
            for k, v in pairs(Config.Locations["trash"]) do
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("inventory:server:OpenInventory", "stash", "fbitrash", {
                                maxweight = 4000000,
                                slots = 300,
                            })
                            TriggerEvent("inventory:client:SetCurrentStash", "fbitrash")
                        end
                    elseif #(pos - v) < 1.5 then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                if player ~= -1 and distance < 2.5 then
                                    local playerId = GetPlayerServerId(player)
                                    TriggerServerEvent("fbi:server:showFingerprint", playerId)
                                else
                                    QBCore.Functions.Notify("No one around!", "error")
                                end
                            end
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Finger scanning")
                        end
                    end
                end
            end
            ...
            for k, v in pairs(Config.Locations["impound"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) <= 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Confiscated vehicle")
                            else
                                if not headerDrawn then
                                    headerDrawn = true
                                    exports['qb-menu']:showHeader({
                                        {
                                            header = 'FBI Confiscation',
                                            params = {
                                                event = 'fbi:client:ImpoundMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
                                ...
                                -- DrawText3D(v.x, v.y, v.z, "~g~E~w~ - FBI Impound")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            ...
            for k, v in pairs(Config.Locations["vehicle"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Almacenar el vehículo")
                            else
                                if not headerDrawn then
                                    headerDrawn = true
                                    exports['qb-menu']:showHeader({
                                        {
                                            header = 'FBI Garage',
                                            params = {
                                                event = 'fbi:client:VehicleMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
